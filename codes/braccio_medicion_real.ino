...
void printPose() {
  ...
  Serial.println(F("Los angulos mostrados son los mandados al servo."));
  Serial.println(F("El sketch no recorta los angulos antes de enviarlos."));
}
...
void resetPose() {
  ...
  applyPose();
  Serial.println();
  Serial.println(F("Pose base enviada."));
  printPose();
}
...
void setup() {
  ...
  Serial.println(F("Medicion real del Braccio con control manual de todas las articulaciones."));
  Serial.println(F("Este sketch esta pensado para medir limites, holgura y referencias reales."));
